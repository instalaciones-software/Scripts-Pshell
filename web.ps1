...
if ($sitiosWeb -eq "yeminus" -or $sitiosWeb -eq "yeminuslocal" -or $sitiosWeb -eq "yeminus2") {
    ...
    # numero de puerto IIS local gateway
    $numeroPort = 8044
            
    $sitiosWeb = $sitiosWeb.ToUpper()

    [System.Environment]::SetEnvironmentVariable("${sitiosWeb}PUERTOLOCAL", "$numeroPort", "Machine")

    [System.Environment]::GetEnvironmentVariable($nombreVariable, "Machine")


    Import-Module WebAdministration

    New-WebBinding -Name $sitiosWeb -Protocol "http" -Port $numeroPort 
