...
<#
    |---------------------------------------------------------------------------------------------------------------------|
    |                                   ESTE SCRIPT EJECUTELO DESDE SSH                                                   |
    | --------------------------------------------------------------------------------------------------------------------|
    #>
...
Script Version 2.0.0.1" -ForegroundColor green
...
$dato = Read-Host " 
    ...
    1. Actualizar version completa
    2. Actualizar parche (ENTER)
...
if ($dato -eq "1") {
    ...
    $toEmail = "instalaciones@yeminus.com,directorsoporte@yeminus.com,instalaciones2@yeminus.com,instalaciones3@yeminus.com,soporte2@yeminus.com,soporte1@yeminus.com,soporte3@yeminus.com,soporte10@yeminus.com,tics@yeminus.com,vquintero@yeminus.com"
    ...
    $body = "Tu código de verificación para realiizar actualizacion completa: $codigo"
    ...
    # Enviar el correo
    $mailmessage = New-Object System.Net.Mail.MailMessage($fromEmail, $toEmail, "Código para Actualizar $sitiosWeb Version $numversion Completa", $body)
    ...
    foreach ($sitioWeb in $nombresSitiosWeb) {
        ...
        if ($sitioWeb -ne "yeminus" -and $sitioWeb -ne "yeminusweb") {
            ...
            $EmailDestinatario = "instalaciones@yeminus.com,directorsoporte@yeminus.com,instalaciones2@yeminus.com,instalaciones3@yeminus.com,soporte2@yeminus.com,soporte1@yeminus.com,soporte3@yeminus.com,soporte10@yeminus.com,tics@yeminus.com,vquintero@yeminus.com,directorsoporte@yeminus.com,instalaciones2@yeminus.com,instalaciones3@yeminus.com,soporte2@yeminus.com,soporte1@yeminus.com,soporte3@yeminus.com,soporte10@yeminus.com,tics@yeminus.com,dguzman@yeminus.com,vquintero@yeminus.com"
...
if ($dato -eq "2" -or $dato -eq "") {
    ...
    $smtpUser = "yeminusinstalaciones@gmail.com"    # Tu correo de Gmail
    $smtpPassword = "qpdx eyum xeci ggci"      # Tu contraseña o aplicación específica de Google
    $toEmail = "instalaciones@yeminus.com,directorsoporte@yeminus.com,instalaciones3@yeminus.com,soporte2@yeminus.com,soporte1@yeminus.com,soporte3@yeminus.com,soporte10@yeminus.com,tics@yeminus.com,vquintero@yeminus.com"
    # Correo al cual enviar el código (puede ser tu propio correo)
    $fromEmail = "yeminusinstalaciones@gmail.com"
    ...
    # Cuerpo del correo con el código de verificación
    $body = "Tu código de verificación para realizar actualizacion Parche: $codigo"
    ...
    # Enviar el correo
    $mailmessage = New-Object System.Net.Mail.MailMessage($fromEmail, $toEmail, "Código para Actualizar $sitiosWeb Version $numversion parche", $body)
    ...
    foreach ($sitioWeb in $nombresSitiosWeb) {
        ...
        if ($program -ne $null -and $program -ne '') {
            ...
            Write-Host " !ATENCIÓN! El sitio web '$sitioWeb' no existe o el nombre es incorrecto." -ForegroundColor Red
               
            break OuterLoop
        }


        $zipArchivo = "C:\inetpub\versiones\4.0.7.0_1.zip"
        $destinoRuta = "C:\inetpub\wwwroot\$sitioWeb"          
        $modulosRuta = "C:\inetpub\wwwroot\$sitioWeb\modules"   
        ...
        foreach ($archivo in $archivosInternos) {
            ...
            if ($archivo.Name -like "*.api.zip") {
                ...
                & $comandoAppCmd\appcmd stop apppool $nombreCarpeta 1>$null
                Expand-Archive -Path $archivo.FullName -DestinationPath $carpetaDestino -Force
                & $comandoAppCmd\appcmd start apppool $nombreCarpeta 1>$null
...
(Get-Content "C:\inetpub\wwwroot\$sitioweb\yeminus\assets\config-resources\config.js") -replace "https://desarrollo.yeminus.com:8080/apiyeminus/", $urlYem | Set-Content "C:\inetpub\wwwroot\$sitioweb\yeminus\assets\config-resources\config.js"
(Get-Content "C:\inetpub\wwwroot\$sitioweb\yeminus\main.09bf5c1c4ec603e4.js") -replace "https://desarrollo.yeminus.com:8080/", $urlYem2  | Set-Content "C:\inetpub\wwwroot\$sitioweb\yeminus\main.09bf5c1c4ec603e4.js"
(Get-Content "C:\inetpub\wwwroot\$sitioweb\yeminus\index.html") -replace "/yeminus/", "/$nombresSitiosWeb/"  | Set-Content "C:\inetpub\wwwroot\$sitioweb\yeminus\index.html"

        Remove-Item  -Force -Recurse "C:\inetpub\wwwroot\$sitioWeb\$sitioWeb" 

        Rename-Item -Path "C:\inetpub\wwwroot\$sitioWeb\yeminus" -NewName "$sitioWeb"

        Remove-Item  -Force -Recurse "C:\inetpub\versiones\"  -Exclude *.zip 
        Remove-Item  -Force  -Recurse "C:\inetpub\wwwroot\$sitioWeb\*.zip"  
    }
