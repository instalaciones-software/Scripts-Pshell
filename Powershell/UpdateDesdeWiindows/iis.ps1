...
if ($dato -eq "1") {
    ...
    if ($sitioweb -ne "yeminus") {
        ...
        & $AppCmdPath add apppool /apppool.name:$sitioWeb /processModel.identityType:"ApplicationPoolIdentity" 1>$null
...
if ($dato -eq "2" -or $dato -eq "") {
    ...
    # Verificar si la version ya está descargada
    if (Test-Path $rutaDescarga) {
        Write-Host "La version $numversion ya está descargada en $rutaDescarga"
    }
    else {
    ...
    foreach ($sitioWeb in $nombresSitiosWeb) {
        ...
        if ($sitioWeb -eq "yeminus" -or $sitioWeb -eq "yeminusweb") {
