...
foreach ($nombreSitioWeb in $nombresSitiosWeb) {
    ...
    foreach ($nombreApi in $listApis) {
         ...
         & "${comandoAppCmd}\appcmd" add vdir /app.name:$nombreSitioWeb/$nombreApi.api /path:/recursos /physicalPath:$rutarecursos /username:"small" /password:$contrasena 1>$null
...
$urlYem = [System.Environment]::GetEnvironmentVariable("${nombreSitioWeb}BASEURLYEMINUS", "Machine")
    ...
    # Write-Host "!ACTUALIZACION COMPLETADA¡" -ForegroundColor White -BackgroundColor darkgreen -NoNewline
    ...
    foreach ($item in $content) {
        foreach ($element in $item) {
          ...
          Invoke-WebRequest $urlYem/Admin.Api/api/configuracion/actualizacionbd/creartablas/$element/YEMINUS
           ...
           Write-Host "ACTUALIZANDO EL ID" $element -ForegroundColor White -BackgroundColor DarkGreen
